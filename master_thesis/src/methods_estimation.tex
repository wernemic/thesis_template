...
\end{algorithmic}
\label{alg:monte}
    \caption{The Monte Carlo simulation pseudocode. For each $\theta, \sigma$ pair, the Monte Carlo simulation first computes the number of particles emitted towards the visible surface of the detector ($A$). Then the function   }
\end{figure}
%  \label{alg:monte}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% %%}
\newpage
\subsection{Sensitivity of detection for \ac{pix} sensor}% %%{
...
The sensitivity matrix $\mathbf{s}^{[t+1]}$ with elements $s_{j}^{[t+1]}$ is computed as follows:
\begin{equation}

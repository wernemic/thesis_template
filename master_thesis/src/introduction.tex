...
This work aims to present a method that performs the fusion of sensory data from multiple \ac{UAV}s (equipped with Compton camera sensor), 
in order to localize multiple sources of ionizing radiation.
The presented method is based on the \ac{MLEM} algorithm that maximizes the likelihood of the measured data.
The localization of radioactive sources is performed online during the flight.
Thanks to that, the group of \ac{UAV}s autonomously adapts its actions to the current estimates in order to increase the precision of the estimate and minimize the search time.
...
Intensity-based detectors are measuring only the number of particles recorded in the sensor (particle flux at the sensor's position).
...
Work presented in \cite{fuku_compton} shows a radioactive hotspot localization inside the \ac{FDNPP} by one \ac{UGV} equipped with Compton camera.
...
The \ac{MLEM} reconstruction method has been originally developed for nuclear medicine imaging, as described in one of the following chapters.
...
present a handheld device for localisation of radioactive sources. 
...
Ground robots can't compete with aerial platforms in the speed of exploration and flexibility, large aerial platforms can't fly close to the obstacle due safety reasons.
...
The section \ref{} describes properties of ionizing radiation and it's interactions with matter, working principle of the Compton camera and \ac{pix} sensor, as well as \ac{ROS} used within this project.
The theoretical background of \ac{MLEM} estimation method is provided in section \ref{}.
Section \ref{} presents the proposed radiation mapping method designed for the \ac{pix}

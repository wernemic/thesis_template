...
The directional sensitivity of the \ac{pix} sensor, modelled via Monte Carlo simulation, is presented in section \ref{chap:mcr}.
The evaluation of the \ac{MLEM} estimation approach, carried out on previously gathered data from experiments with real radioactive sources, is shown in section \ref{chap:exp1}.
The functionality of the whole system (estimation method together with the proposed search strategy) in simulation is presented in section \ref{chap:exp2}.
Finally, the whole system was also tested on real hardware with simulated radioactive sources, as shown in section \ref{chap:exp3}.
...
Results of the Monte Carlo simulations are shown in figure \ref{fig:monte_clar}.
The corresponding geometry of the \ac{CdTe} block is shown in figure \ref{fig:monte_axes}.

The \ac{pix} sensor's directional sensitivity is depicted in figure \ref{fig:monte_finalll}.
...
The probability of reaching the sensor's surface (given the solid angle of the sensor from different positions) is shown in figure \ref{fig:monte_reaching}.
...
Therefore a photon reaching the sensor's surface from the side more likely causes the Compton measurement, as can be seen in figure \ref{fig:monte_cone_from_hitting}.
...
\begin{figure}[!htb]% %%{
  \subfloat[$p(\mathrm{cone\ detected})$] {
    \centering
    \includegraphics[width=0.45\textwidth,trim={1cm 1cm 2.5cm 1cm},clip]{./fig/photos/monte_carlo_final_prob.eps}
  ...
  \subfloat[$p(\mathrm{reaching\ the\ sensor})$] {
    \includegraphics[width=0.45\textwidth,trim={1cm 1cm 1cm 1cm},clip]{./fig/photos/monte_carlo_total_activity.eps}
    \label{fig:monte_reaching}
  }
  \subfloat[$p(\mathrm{cone\ detected}\ |\ \mathrm{reaching\ the\ sensor})$] {
    \includegraphics[width=0.45\textwidth,trim={1cm 1cm 1cm 1cm},clip]{./fig/photos/monte_carlo_fraction.eps}
  ...
  The probability that a $\SI{662}{\kilo\electronvolt}$ photon emitted at the corresponding position causes a Compton cone is depicted in \ref{fig:monte_finalll}. 
  The probability that the particle emitted at a given position reaches the detector is depicted in \ref{fig:monte_reaching}. 
  Lastly, the probability that a photon (that reached the sensor) causes a Compton cone is illustrated in figure \ref{fig:monte_cone_from_hitting}.}
...
Four sources of Cesium-137 with activity $1900, 500, 180, 50\ \si{\mega\becquerel}$ were located at positions shown in figure \ref{e1:gt}.
...
The results of the proposed \ac{MLEM} estimation method are presented in figure \ref{fig:e1_all}.
...
The final estimate of the emission intensity $\bm{\lambda}$ (based on all data acquired during the experiment) is presented in figure \ref{e1:lam}. 
The sensitivity of detection (figure \ref{e1:sen}) confirms what was stated before - the drones were mostly encircling the $\SI{500}{\mega\becquerel}$ source at position $(-2, -10)$, which was also correctly localized.
...
The convergence of \ac{MLEM} algorithm (using \ac{MSE} as the metric) is shown in figure \ref{e1:mse}. 
...
The iterative \ac{MLEM} algorithm is initialized with back-projection (figure \ref{e1:bp}). 
We can see in figure \ref{e1:mse} that the decrease of the $MSE$ stopped after a few iterations.
...
\begin{figure}[!htb]% %%{
  \centering
  \subfloat[estimate of ionizing radiation intensity $\bm{\lambda}$ (normalized)] {
...
In summary, the comparison of \ac{MLEM} estimate (figure \ref{e1:lam}) and back-projection (figure \ref{e1:bp}) show that the \ac{MLEM} significantly improved the reconstruction quality.
...
The simulation process (visualized in \textit{RViz}) is shown in figure \ref{fig:simu_search}.
...
\subsection{Results}
The results of the \ac{MLEM} reconstruction are presented in figure \ref{fig:e2}.
The progress of radiation mapping is illustrated in figures \ref{e2:lam1} (time $t = \SI{25}{\second}$) and \ref{e2:lam2} ($t = \SI{100}{\second}$), corresponding sensitivity of detection is presented in \ref{e2:sen1}, \ref{e2:sen2}.
...
The \ac{MLEM} method significantly improved the solution compared to the back-projection illustrated in figure \ref{e2:bp}.
...
\begin{figure}[!htb]% %%{
  \centering
  \subfloat[($t = \SI{25}{\second}$) source intensities $\bm{\lambda}$ (normalized) ] {
  ...
  \newline
  \noindent
  \subfloat[($t = \SI{100}{\second}$) source intensities $\bm{\lambda}$ (normalized) ] {
  ...
  The progress of the \ac{MLEM} reconstruction method is shown in \ref{e2:lam1}, \ref{e2:sen1} and \ref{e2:lam2}, \ref{e2:sen2}.
  %The output of the \ac{MLEM} method after $\SI{25}{\second}$ (\ref{e2:lam1}, \ref{e1:sen1}) and after $\SI{100}{\second}$ (\ref{e2:lam2}, \ref{e1:sen2}) is presented. 
  %We can see that the quality of the \ac{MLEM} estimate improved during the flight thanks to the active search strategy.
  The back-projection of all cones measured in the first $\SI{100}{\second}$ is shown in \ref{e2:bp}.  }
  \label{fig:e2}
\end{figure}% %%}
...
\begin{figure}[!htb]
  ...
  \caption{Field experiments with real hardware. The drones \ref{fig:d1} were mapping the simulated radioactive sources located somewhere in the open field \ref{fig:d2}}
  \label{fig:}
\end{figure}
\subsection{Experiment setup}
...
The viewpoints (positions of the drones) were sampled with $\SI{5}{\hertz}$, and the \ac{MLEM} estimate was updated every $\SI{2}{\second}$.
Four simulated sources of $\SI{662}{\kilo\electronvolt}$ photons with activity $2000, 1000, 1000, 500\ \si{\mega\becquerel}$ were located at positions shown in figure \ref{e3:gt}.
...
\subsection{Results}
In the initial phase (from time $t = \SI{0}{\minute}$ to time $t = \SI{2}{\minute}$), the drones flew over the area once in a ``zig-zag'' pattern following the predefined trajectory (covering the area uniformly, as can be seen in figure \ref{e3:senzig}) and measured first $17$ Compton cones.
The ``zig-zag'' trajectory was precomputed using the MRS UAV System \cite{mrs_system}.
The \ac{MLEM} estimate and sensitivity of detection after the initial phase is shown in figures \ref{e3:lamzig} and \ref{e3:senzig}.
...
The trajectories of the \ac{UAV}s during the search phase (figure \ref{e3:paths}) show that the two ``exploitation`` drones focused on acquiring more measurements while the third drone was exploring the unexplored area.
The final estimate of radiation sources intensities (at the end of the experiment $t = \SI{10}{\minute}$) is shown in figure \ref{e3:lam}.
...
The weakest $\SI{500}{\mega\becquerel}$ source at position ($75, 75$) was not discovered despite the fact that the exploration drone flew around it multiple times (see blue path in figure \ref{e3:paths}).
...
Although the maximum likelihood does not provide an accurate estimate for a small number of cones (see \ac{MLEM} estimate in figure \ref{e3:lamzig} generated from 17 Compton cones), the inaccurate estimate ``attracts'' the \ac{UAV}s that come closer and likely measure more ionizing photons, that guide the \ac{UAV}s to the actual position of the source.

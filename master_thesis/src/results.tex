...
The source is considered as detected if the estimated emission activity ($\bm{\lambda}$ rescaled to $0-1$ range over all map) at the ground-truth source position exceeds detection threshold $0.5$ (in the vicinity of $\SI{2}{\meter}$ around the ground-truth position) and remains above threshold $0.3$ during the rest of the experiment.
...
\begin{figure}[!htb]
  ...
  \subfloat [Ground truth positions]{
    \includegraphics[width=0.4\textwidth,,trim={0cm 0 1.5cm 1.2cm}, clip ]{./fig/photos/auto_eval_gt.eps}
    \label{fig:eval4}
  }
  \caption{The number of correctly detected sources with the same emission activity (located at positions \protect\subref{fig:eval4}). Dashed lines represent the individual experiments (each scenario was repeated $5$ times), the black line is the average. }
  \label{fig:eval}
\end{figure}
\subsubsection{Results}
